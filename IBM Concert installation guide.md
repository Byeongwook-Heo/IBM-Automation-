...
## 2.1 단계별 산출물/완료 기준 (추가)

| 단계 | 필수 산출물 | 완료(Go) 기준 |
|---|---|---|
| 요청/범위 | 범위정의서, 대상 시스템 목록 | 영향 시스템/책임자 승인 완료 |
| 설계/승인 | 아키텍처 다이어그램, 포트 정책서, 백업정책서 | CAB/보안 승인 티켓 완료 |
| 구축/설치 | 설치 로그, 파라미터 파일, 버전 정보 | 서비스 기동 + 헬스체크 성공 |
| 검증/UAT | 테스트 결과서, 결함 조치내역 | 주요 시나리오 Pass율 100% |
| 운영 전환 | Cutover 체크리스트, 롤백 플랜 | Go/No-Go 회의 승인 |

## 2.2 Go / No-Go 게이트 (추가)

운영 반영 직전 아래 항목 중 1개라도 미충족이면 **No-Go** 처리합니다.

- [ ] 관리자/일반 사용자 로그인 및 권한 분리 검증 완료
- [ ] DB 백업/복원 샘플 테스트 성공
- [ ] 모니터링/알림(메일 또는 웹훅) 테스트 성공
- [ ] 장애 시 롤백 절차 리허설 완료
- [ ] 운영자 인수인계(온콜 포함) 완료
...
# 번들 목록 확인
tar -tf concert-bundle.tar | head -n 50

# 이미지 tar 로드 예시(런타임 환경에 맞게 사용)
# docker load -i concert-images.tar
# ctr -n k8s.io images import concert-images.tar
```
